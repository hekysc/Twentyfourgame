...
- 本局计时：0.1s 精度；错答继续计时，答对/查看答案/跳过立即停表；超过 2 分钟只提醒
- 在线挑战：按 52 张牌预发题目；客户端本地判题、计时和切题，成绩持久化后后台批量核验同步
- 牌堆进度：剩余数量显示为 `?/52`；未关闭题库 30 天无活动后回收，牌堆关闭后保留 7 天上传宽限期，历史成绩独立保存
...
- 重洗规则：
  ...
  - 超时提醒：超过 2 分钟只提示，继续显示累计用时；错误尝试不计入成绩，答对记成功，查看答案或跳过记失败。
  - 答对后显示 0.5 秒成功反馈再切题；网络等待不计入页面成绩，新题就绪后从 0 开始计时。
  - 查看答案后留在当前题学习；之后点下一题不会再次计失败。计时单元中的“重新发牌”按跳过处理。
